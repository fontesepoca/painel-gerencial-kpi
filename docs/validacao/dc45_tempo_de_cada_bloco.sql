-- dc45 — O tempo de cada bloco, cronometrado dentro do banco.
--
-- A dc44 devolveu os valores, mas o que interessa é o TEMPO, e a ferramenta nem sempre mostra.
-- Este bloco PL/SQL mede sozinho e imprime os três tempos de uma vez.
--
-- POR QUE `SUM(todas as colunas)` E NÃO `COUNT(*)`
--   `COUNT(*)` sobre uma subconsulta deixa o Oracle eliminar colunas — e, com elas, joins que
--   ninguém referencia. O plano medido deixaria de ser o plano real, e o número seria bonito e
--   inútil. Somando todas as colunas agregadas, cada bloco é obrigado a produzir tudo o que
--   produz na consulta de verdade.
--
-- O QUE JÁ SABEMOS, e que este script não repete
--   A subconsulta `esp` tem 54 linhas (dc44) — está descartada como suspeita.
--   O período move 131.684 notas e 1.570.379 itens de PCMOV.
--
-- COMO RODAR
--   Precisa de saída de servidor ligada. No SQL*Plus ou SQL Developer:  SET SERVEROUTPUT ON
--   Se a sua ferramenta não mostrar DBMS_OUTPUT, me avise que eu devolvo isto como SELECT.
--
--   Rode DUAS VEZES. A segunda passada mostra o que o cache não resolve — na dc43, o
--   faturamento inteiro caiu de 247 s para 133 s, e essa metade que sobra é o alvo.
--
-- O QUE ME DEVOLVER
--   As duas saídas, inteiras. São seis linhas de texto.
...
BEGIN
  ...
  SELECT SUM(VLCUSTOFIN + VLVENDA + VLTABELA + VLST + VLPIS + VLCOFINS)
    INTO v_total
    FROM (
  ...
  anotar('bloco 1 - vendas por item', v_inicio);

  -- ── BLOCO 2 — devoluções ─────────────────────────────────────────────────
  v_inicio := DBMS_UTILITY.GET_TIME;

  SELECT SUM(VLDEVOLUCAO + VLCMVDEVOL + VLST_DEV + NVL(VLPIS_DEV,0) + NVL(VLCOFINS_DEV,0))
    INTO v_total
    FROM (
  ...
  anotar('bloco 2 - devolucoes', v_inicio);

  -- ── BLOCO 3 — notas sem item ─────────────────────────────────────────────
  v_inicio := DBMS_UTILITY.GET_TIME;

  SELECT SUM(VLCUSTOFIN + VLVENDA + VLTABELA)
    INTO v_total
    FROM (
      SELECT TO_CHAR(NF.DTSAIDA,'mm/yyyy') AS MESANO,
  ...
  anotar('bloco 3 - notas sem item', v_inicio);
END;
/
